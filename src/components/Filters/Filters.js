...
import { searchValueChanged, statusChanged, priorityChanged } from "../../redux/actions";
        ...
        <div className={cx("filters")}>
                ...
                <div className={cx("status__filter")}>
                    {
                        statusFilter.map(status => (
                            <div className={`status__filter__${status.name}`}>
                                <input
                                    type="radio"
                                    name={status.name}
            ...
            <div className={cx("priority")}>
                ...
                <div className={cx("priority__filter")}>
                    <select name="priority" onChange={handlePriorityChanged}>
                        {
                            priorityFilter.map(item => {
                                const selected = item.value === filters.priority ? true : false;
                                return <option selected={selected} value={item.value} >{item.name}</option>
                            })
                        }
                    </select>
                </div>
